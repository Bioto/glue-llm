...
- **Customisable retry behaviour**: `RetryConfig` for per-client and per-call retry control. Use `retry_enabled=False` (or `retry_config=RetryConfig(retry_enabled=False)`) to disable retries; use `retry_on=[ExceptionType, ...]` to restrict retries to specific exception types; use `callback` to decide per error whether to retry and to inject model params (e.g. `temperature`) for the next attempt. When `callback` is set, it takes precedence over `retry_on`. Available on `GlueLLM.__init__`, `complete()`, `structured_complete()`, and the module-level `complete` and `structured_complete` functions.
